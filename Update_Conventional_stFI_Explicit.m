function [F,Updated] = Update_Conventional_stFI_Explicit(F,cdt,MeshParam,Area,Updated)

cdt_subgrid=cdt/MeshParam.UpdateNum_subgrid;

[F]=Update_E_CoarseRegion(F,cdt,MeshParam,Area);
[F,Updated]=Update_E_RegionBoundary_FirstTimeSec(F,cdt,cdt_subgrid,MeshParam,Area,Updated);
...
%% 
function [F,Updated]=Update_E_RegionBoundary_FirstTimeSec(F,cdt,cdt_subgrid,MeshParam,Area,Updated)
...
localiIdx=1;
% Updated(i,j)=Updated(i,j)+1;
F(i,j).Ex(localiIdx,localjIdx)=(Area.E_innercorner/(sqrt(10)/4.0))*( ...
    ((sqrt(10)/4.0)/Area.E_innercorner)*F(i,j).Ex(localiIdx,localjIdx) ...
...
F(i,j).Ex(localiIdx,localjIdx)=(Area.E_innercorner/(sqrt(10)/4.0))*( ...
    ((sqrt(10)/4.0)/Area.E_innercorner)*F(i,j).Ex(localiIdx,localjIdx) ...
    + (cdt        /Area.Bz_next2outercorner)*F(i,j  ).Bz ...
    - (cdt_subgrid/Area.Bz_innercorner     )*F(i,j-1).Bz(localiIdx,localjIdx+1)...
    );
F(i,j).ExPreserve=F(i,j).ExPreserve+F(i,j).Ex(localiIdx,localjIdx);
localiIdx=2;
% Updated(i,j)=Updated(i,j)+1;
...
localjIdx=1;
% Updated(i,j)=Updated(i,j)+1;
F(i,j).Ey(localiIdx,localjIdx)=(Area.E_innercorner/(sqrt(10)/4.0))*( ...
    ((sqrt(10)/4.0)/Area.E_innercorner)*F(i,j).Ey(localiIdx,localjIdx) ...
    - (cdt_subgrid/Area.Bz_innercorner     )*F(i  ,j).Bz(localiIdx,localjIdx) ...
    + (cdt        /Area.Bz_next2outercorner)*F(i-1,j).Bz...
    );
...
localjIdx=1;
% Updated(i,j)=Updated(i,j)+1;
F(i,j).Ey(localiIdx,localjIdx)=(Area.E_insideboundary_larger/0.5)*( ...
    (0.5/Area.E_insideboundary_larger)*F(i,j).Ey(localiIdx,localjIdx) ...
    - (cdt_subgrid/Area.Bz_insideboundary)*F(i  ,j).Bz(localiIdx  ,localjIdx)...
    + (cdt_subgrid/Area.Bz_insideboundary)*F(i-1,j).Bz(localiIdx+1,localjIdx) ...
    );
localjIdx=2;
% Updated(i,j)=Updated(i,j)+1;
F(i,j).Ey(localiIdx,localjIdx)=((0.5*cdt_subgrid)/0.5)*( ...
...
for j=MeshParam.Fine_Y_from+1:MeshParam.Fine_Y_to-1
    ...
    for localjIdx=1:2
        F(i,j).Ey(localiIdx,localjIdx)=((0.5*cdt_subgrid)/0.75)*( ...
            (0.75/(0.5*cdt_subgrid))*F(i,j).Ey(localiIdx,localjIdx) ...
            + (cdt_subgrid/Area.Bz_insideboundary )*F(i-1,j).Bz(2,localiIdx) ...
            );
        F(i,j).EyPreserve=F(i,j).EyPreserve+F(i,j).Ey(localiIdx,localjIdx);
    end
end
